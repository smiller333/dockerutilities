...
// TestDockerClient_ListContainers tests listing containers
// Note: This test requires a running Docker daemon
func TestDockerClient_ListContainers(t *testing.T) {
	...
	containers, err := client.ListContainers(ctx, true) // List all containers
	if err != nil {
		t.Fatalf("ListContainers() error = %v", err)
	}

	// We don't know how many containers exist, so just check that we got a slice
	if containers == nil {
		t.Error("ListContainers() returned nil")
	}
}

// TestDockerClient_ListImages tests listing images
// Note: This test requires a running Docker daemon
func TestDockerClient_ListImages(t *testing.T) {
	if testing.Short() {
		t.Skip("skipping integration test in short mode")
	}
	...
	images, err := client.ListImages(ctx, true) // List all images
	if err != nil {
		t.Fatalf("ListImages() error = %v", err)
	}

	// We don't know how many images exist, so just check that we got a slice
	if images == nil {
